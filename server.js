...
const { deleteActivity, createActivity, getActivities } = require('./activities.js');
const { deleteRoutine, createRoutine, getRoutines } = require('./routines.js');
...
app.delete('/api/v1/activities/:id', async (req, res) => {
  ...
  if(activity){
  deleteActivity(id);
  res.send(`<h1>activity deleted</h1>`);
  }
});
...
app.delete('/api/v1/routines/:id', async (req, res) => {
  ...
  if(routine){
  deleteRoutine(id);
  res.send(`<h1>routine deleted</h1>`);
  }
});
